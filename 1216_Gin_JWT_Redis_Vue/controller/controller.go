...
func Register(c *gin.Context) {
	...
	// 確認帳號是否存在，不存在就Create
	checkOK, err := util.GetUsername(reg.Username)
	if err != nil {
	...
	if !checkOK {
		...
		if err != nil {
			ErrMsg(c, Code_DB_Conn, "ˊ註冊失敗，請重新嘗試", nil, err)
			return
		}
		OkMsg(c, "註冊成功", nil)
	} else {
...
func Login(c *gin.Context) {
	...
	// 確認帳號是否存在
	checkOK, err := util.GetUsername(login.Password)
	if err != nil {
		ErrMsg(c, Code_Param_Invalid, "Redis Exists", nil, err)
		return
	}
	...
	if checkOK {
		...
		UserData = db.User{Username: login.Username, Password: value}
